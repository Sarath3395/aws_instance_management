...
def check_spot_or_not(ec2_con_re):
    ...
    instances = ec2.instances.filter(Filters=[
        {'Name': 'instance-state-name', 'Values': ['running', 'stopped', 'terminated', 'shutting-down', 'pending']}])
    spot_ins = []
    ...
    for each in list:
        ...
        if ((prs == "terminated") & (spot_in == 1)):
            ...
            if (instance_creation.objects.filter(terminated_id=each).exists()) == 0:
                ...
                if ((ami_creation.objects.filter(instance_id=each).exists() == 1) & (
                        new_ami_det['state'] == 'available')):
